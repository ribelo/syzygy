...
type AppStorage = Storage<NotificationModel, Storage<CacheModel, Storage<AppConfigModel, Storage<UserModel, EmptyStorage>>>>;
    ...
    match event {
        AppEvent::UserLogin { username, password } => {
            ...
            Command::<AppEvent, AppEffect>::batch([
                Command::<AppEvent, AppEffect>::effect(AppEffect::AuthenticateUser { username, password }),
                Command::<AppEvent, AppEffect>::effect(AppEffect::LogActivity { action: "login_attempt".to_string() }),
        ...
        AppEvent::UserLogout => {
            ...
            Command::<AppEvent, AppEffect>::batch([
                ...
                Command::<AppEvent, AppEffect>::effect(AppEffect::LogActivity { action: "logout".to_string() }),
        ...
        AppEvent::UpdateProfile { name, email } => {
            ...
            Command::<AppEvent, AppEffect>::batch([
                ...
                Command::<AppEvent, AppEffect>::effect(AppEffect::UpdateUI { component: "profile".to_string() }),
                Command::<AppEvent, AppEffect>::event(AppEvent::AddNotification { 
        ...
        AppEvent::ChangeTheme { theme } => {
            ...
            Command::<AppEvent, AppEffect>::batch([
                Command::<AppEvent, AppEffect>::effect(AppEffect::SaveConfig),
                Command::<AppEvent, AppEffect>::effect(AppEffect::UpdateUI { component: "theme".to_string() }),
            ])
        }
        ...
        AppEvent::AddNotification { message } => {
            ...
            Command::<AppEvent, AppEffect>::effect(AppEffect::UpdateUI { component: "notifications".to_string() })
        }
        ...
        AppEvent::LoadCachedData { key } => {
            ...
            if cache.cached_data.contains_key(&key) {
                ...
                Command::<AppEvent, AppEffect>::effect(AppEffect::LoadFromDatabase { table: "data".to_string() })
            }
        }
        ...
        AppEvent::InitializeApp => {
            ...
            Command::<AppEvent, AppEffect>::batch([
                Command::<AppEvent, AppEffect>::effect(AppEffect::LoadFromDatabase { table: "user_preferences".to_string() }),
                Command::<AppEvent, AppEffect>::effect(AppEffect::NetworkRequest { url: "https://api.example.com/config".to_string() }),
                Command::<AppEvent, AppEffect>::event(AppEvent::AddNotification { 
        ...
        AppEvent::RefreshData => {
            ...
            if refresh_needed {
                Command::batch([
                    Command::<AppEvent, AppEffect>::effect(AppEffect::NetworkRequest { url: "https://api.example.com/refresh".to_string() }),
                    Command::<AppEvent, AppEffect>::effect(AppEffect::LoadFromDatabase { table: "all_data".to_string() }),
                    Command::<AppEvent, AppEffect>::event(AppEvent::AddNotification { 
...
fn bench_realistic_event_processing(c: &mut Criterion) {
    ...
    group.bench_function("user_login", |b| {
        b.iter(|| {
            ...
            let mut ctx: EventContext<AppEvent, AppEffect, AppStorage> = EventContext::new(&mut storage);
    ...
    group.bench_function("theme_change", |b| {
        b.iter(|| {
            let mut storage = storage.clone();
            let mut ctx: EventContext<AppEvent, AppEffect, AppStorage> = EventContext::new(&mut storage);
            let event = AppEvent::ChangeTheme { theme: "dark".to_string() };
            black_box(realistic_update(black_box(event), &mut ctx))
        })
    });
...
fn bench_command_operations(c: &mut Criterion) {
    ...
    group.bench_function("single_effect", |b| {
        b.iter(|| {
            black_box(Command::<AppEvent, AppEffect>::effect(AppEffect::LogActivity { 
    ...
    group.bench_function("single_event", |b| {
        b.iter(|| {
            black_box(Command::<AppEvent, AppEffect>::event(AppEvent::AddNotification { 
    ...
    group.bench_function("batch_commands", |b| {
        b.iter(|| {
            black_box(Command::<AppEvent, AppEffect>::batch([
                Command::<AppEvent, AppEffect>::effect(AppEffect::SaveConfig),
                Command::<AppEvent, AppEffect>::effect(AppEffect::UpdateUI { component: "test".to_string() }),
                Command::<AppEvent, AppEffect>::event(AppEvent::AddNotification { message: "done".to_string() }),
            ]))
        })
    });
    ...
    group.bench_function("nested_batch_commands", |b| {
        b.iter(|| {
            black_box(Command::batch([
                Command::batch([
                    Command::<AppEvent, AppEffect>::effect(AppEffect::SaveConfig),
                    Command::<AppEvent, AppEffect>::effect(AppEffect::LogActivity { action: "nested".to_string() }),
                ]),
                Command::batch([
                    Command::<AppEvent, AppEffect>::event(AppEvent::AddNotification { message: "test".to_string() }),
                    Command::<AppEvent, AppEffect>::effect(AppEffect::UpdateUI { component: "ui".to_string() }),
                ]),
            ]))
        })
    });
...
fn bench_storage_patterns(c: &mut Criterion) {
    ...
    group.bench_function("mixed_access_pattern", |b| {
        b.iter(|| {
            ...
            black_box((user.id, config.theme.len(), notifications.unread_count, cache.cache_hits))
        })
    });
