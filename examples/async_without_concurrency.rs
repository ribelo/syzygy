...
fn update(event: Event, ctx: &mut EventContext<Event, Effect, Model>) -> Command<Event, Effect> {
    ...
    match event {
        Event::SaveFiles => {
            ...
            Command::batch([
                Command::effect(Effect::SaveFile {
...
fn handle_effects(effect: Effect, ctx: &EffectContext<Event, ()>) -> Task<Event, ()> {
    match effect {
        Effect::SaveFile { name } => {
            // Using ctx.spawn - provides access to context for resources, events, etc.
            ctx.spawn::<InlineAsync<Event>, _, _>(move |_ctx| async move {
                println!("📝 Saving {name}...");
                // In real code, you could use ctx here to:
                // - Access resources: ctx.resources()
                // - Send events: ctx.send_event()
                // - Spawn more tasks: ctx.spawn()
                println!("✅ Saved {name}");
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    runner.core_mut().send_event(Event::SaveFiles);
