            ...
            let _config_plan =
                Task::async_task_with::<TokioExecutor, _, _>(|_ctx: EffectContext<AppEvent, ()>| {
                    async move {
                        ...
                        Outcome::Event(AppEvent::ConfigLoaded(config))
                    }
                    .boxed()
                });
            ...
            let _user_data_plan =
                Task::async_task_with::<TokioExecutor, _, _>(|_ctx: EffectContext<AppEvent, ()>| {
                    async move {
                        ...
                        Outcome::Event(AppEvent::UserDataLoaded(user_data))
                    }
                    .boxed()
                });
            ...
            let _bootstrap_plan =
                Task::async_task_with::<TokioExecutor, _, _>(move |_ctx: EffectContext<AppEvent, ()>| {
                    async move {
                        ...
                        Outcome::Events(vec![config_result, user_data_result, AppEvent::BootstrapComplete])
                    }
                    .boxed()
                });
        ...
        AppEffect::TryMirror { url } => {
            Task::async_task_with::<TokioExecutor, _, _>(move |_ctx: EffectContext<AppEvent, ()>| {
                ...
                async move {
                    ...
                    Outcome::Event(AppEvent::MirrorSelected(url))
                }
                .boxed()
            })
        }
        ...
        AppEffect::ProcessWorkflowStep { step, item } => {
            Task::async_task_with::<TokioExecutor, _, _>(move |_ctx: EffectContext<AppEvent, ()>| {
                ...
                async move {
                    ...
                    Outcome::Event(AppEvent::WorkflowStepComplete(step))
                }
                .boxed()
            })
        }
        ...
        AppEffect::Cleanup => {
            Task::async_task_with::<TokioExecutor, _, _>(move |_ctx: EffectContext<AppEvent, ()>| {
                async move {
                    ...
                    Outcome::Event(AppEvent::LogMessage("Cleanup completed".to_string()))
                }
                .boxed()
            })
        }
