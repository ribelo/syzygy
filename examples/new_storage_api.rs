...
fn update(
    event: AppEvent, 
    ctx: &mut EventContext<AppEvent, AppEffect, Storage<CounterModel, Storage<ConfigModel, Storage<UserModel, EmptyStorage>>>>
    ...
    match event {
        ...
        AppEvent::ToggleNotifications => {
            let (theme, notifications) = {
                ...
                println!("Notifications: {}", if config.notifications { "ON" } else { "OFF" });
...
async fn handle_effects(
    ...
    match effect {
        ...
        AppEffect::SaveConfig { theme, notifications } => {
            println!("💾 Saving config: theme={}, notifications={}", theme, notifications);
            // Simulate async work
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let (mut core, shell) = Syzygy::builder::<AppEvent, AppEffect>()
        .model(UserModel {
    ...
    println!("  User: {} <{}> (logins: {})", user.name, user.email, user.login_count);
    println!("  Config: theme={}, notifications={}, version={}", 
    ...
    runner.core_mut().send_event(AppEvent::ToggleNotifications)?;
