        ...
        match event {
            UserEvent::Login { username } => {
                Command::effect(UserEffect::ValidateCredentials { username })
                    .then(Command::effect(UserEffect::LogActivity { 
...
fn main() {
    ...
    println!("   Separated {} events and {} effects", events.len(), effects.len());
    ...
    match validated_result {
        Ok(validated_command) => {
            println!("   Validation successful: {} outputs", validated_command.len());
        }
        Err(error) => {
