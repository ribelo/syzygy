    ...
    _ctx: &mut EventContext<SequentialEvent, SequentialEffect, Storage<SequentialModel, EmptyStorage>>,
        ...
        match event {
            SequentialEvent::StartSequence => {
                ...
                Command::sequence([
                    Command::effect(SequentialEffect::Step { id: 1, duration_ms: 100 }),
                    Command::effect(SequentialEffect::Step { id: 2, duration_ms: 100 }),
                    Command::effect(SequentialEffect::Step { id: 3, duration_ms: 100 }),
                ])
            }
        }
}
...
async fn test_sequential_effects_are_actually_sequential() {
    ...
    // Start the sequence
    runner.core().send_event(SequentialEvent::StartSequence).unwrap();
    
    // Process effects and wait for completion
    for _ in 0..50 { // Give plenty of time for effects to complete
    ...
    println!("  Step 1 ended at: {:?}", step1_end.duration_since(overall_start));
    println!("  Step 2 started at: {:?}", step2_start.duration_since(overall_start));
    println!("  Step 3 started at: {:?}", step3_start.duration_since(overall_start));
    ...
    assert!(overall_duration >= Duration::from_millis(250), 
            "BUG: Total execution too fast ({:?}), indicates parallel execution", overall_duration);
}
...
async fn test_parallel_baseline_for_comparison() {
    ...
    let parallel_command = Command::parallel([
        SequentialEffect::Step { id: 1, duration_ms: 100 },
        SequentialEffect::Step { id: 2, duration_ms: 100 },
        SequentialEffect::Step { id: 3, duration_ms: 100 },
    ]);
    ...
    loop {
        runner.tick(syzygy::spawn::spawner()).await.unwrap();
        tokio::time::sleep(Duration::from_millis(10)).await;
        if execution_log.lock().unwrap().len() >= 3 { break; }
    ...
    assert!(overall_duration < Duration::from_millis(200), 
