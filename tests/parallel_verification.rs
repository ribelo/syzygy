    ...
    _ctx: &mut EventContext<ParallelEvent, ParallelEffect, Storage<ParallelModel, EmptyStorage>>,
        ...
        match event {
            ParallelEvent::StartParallel => {
                ...
                Command::parallel([
                    ParallelEffect::Step { id: 1, duration_ms: 100 },
                    ParallelEffect::Step { id: 2, duration_ms: 100 },
                    ParallelEffect::Step { id: 3, duration_ms: 100 },
                ])
            }
        }
}
...
async fn test_parallel_effects_are_actually_parallel() {
    ...
    runner.core().send_event(ParallelEvent::StartParallel).unwrap();
    
    // Process effects and wait for completion
    for _ in 0..30 { // Give time for effects to complete
    ...
    for (id, start, end) in &sorted_log {
        println!("  Step {}: {:?} -> {:?} (duration: {:?})", 
                 id, start.duration_since(overall_start), end.duration_since(overall_start),
    ...
    for (i, start_time) in start_times.iter().enumerate() {
        ...
        println!("  Step {} started {} after Step 1", i + 1, format!("{:?}", start_diff));
        
        assert!(start_diff < Duration::from_millis(50), 
    ...
    let actual_execution_time = sorted_log.iter()
    ...
    assert!(actual_execution_time <= Duration::from_millis(120), 
            "Individual effect took too long ({:?}), timing issue", actual_execution_time);
...
async fn test_mixed_sequential_and_parallel() {
    ...
    let mixed_command = Command::batch([
        ...
        Command::parallel([
            ParallelEffect::Step { id: 10, duration_ms: 80 },
            ParallelEffect::Step { id: 11, duration_ms: 80 },
        ]),
        // Then: sequential execution (should complete in ~160ms total)
        Command::sequence([
            Command::effect(ParallelEffect::Step { id: 20, duration_ms: 80 }),
            Command::effect(ParallelEffect::Step { id: 21, duration_ms: 80 }),
        ]),
    ]);
    ...
    let parallel_effects: Vec<_> = log.iter().filter(|(id, _, _)| *id >= 10 && *id < 20).collect();
    if parallel_effects.len() == 2 {
        ...
        assert!(start_diff < Duration::from_millis(30), "Parallel effects didn't start together");
    }
    ...
    if sequential_effects.len() == 2 {
        ...
        println!("  Sequential: first ended at {:?}, second started at {:?}", 
        ...
        assert!(second_start >= earliest_acceptable_start,
