...
mod tokio_tests {
    ...
    fn scheduler_panics_with_helpful_message_when_no_tokio_runtime() {
        ...
        assert!(result.is_err(), "scheduler() should panic when no tokio runtime exists");
    ...
    fn scheduler_strict_returns_error_when_no_tokio_runtime() {
        ...
        assert!(result.is_err(), "scheduler_strict() should return error when no tokio runtime exists");
    ...
    async fn scheduler_strict_returns_scheduler_when_tokio_runtime_exists() {
        ...
        let result = scheduler_strict();
        assert!(result.is_ok(), "scheduler_strict() should succeed when tokio runtime exists");
    }
}
...
mod no_tokio_tests {
    ...
    fn scheduler_strict_returns_error_when_no_tokio_feature() {
        ...
        let result = scheduler_strict();
        assert!(result.is_err(), "scheduler_strict() should return error when tokio feature disabled");
    }
}