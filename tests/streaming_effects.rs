...
use syzygy::prelude::*;
use syzygy::streaming::{EffectResult, consume_effect_output};
...
) -> EffectResult<StreamEvent> {
    match effect {
        StreamEffect::ProduceSingle { value } => {
            sleep(Duration::from_millis(10)).await;
            EffectResult::Single(StreamEvent::Data(value))
        }
        ...
        StreamEffect::JoinFutures => {
            sleep(Duration::from_millis(10)).await;
            EffectResult::Single(StreamEvent::Data("joined".to_string()))
        }

        StreamEffect::RaceFutures => {
            sleep(Duration::from_millis(5)).await;
            EffectResult::Single(StreamEvent::Data("raced".to_string()))
        }
...
async fn test_consume_effect_output_helper() {
    ...
    let (tx, rx) = unbounded();
    let ctx = EffectContext::new(Some(tx), syzygy::storage::EmptyStorage, syzygy::executor::EmptyExecutorStorage);

    // Test consuming a single event
    let output = EffectResult::Single(StreamEvent::Data("test".to_string()));
    ...
    let output = EffectResult::Stream(stream);
    consume_effect_output(output, &ctx).await;
