    ...
    let model: &mut CompositionModel = ctx.model_mut();
        match event {
            CompositionEvent::StartWorkflow => {
                ...
                Command::sequence([
                    Command::effect(CompositionEffect::DelayedLog { message: "Step 1".to_string(), delay_ms: 10 }),
                    Command::effect(CompositionEffect::DelayedLog { message: "Step 2".to_string(), delay_ms: 10 }),
                    Command::effect(CompositionEffect::DelayedLog { message: "Step 3".to_string(), delay_ms: 10 }),
                ])
            }
...
async fn test_sequential_effects_execution_order() {
    ...
    // Start the workflow
    runner.core().send_event(CompositionEvent::StartWorkflow).unwrap();
    ...
    loop {
        ...
        if start_time.elapsed() > timeout {
            panic!("Test timeout - only {} steps completed", runner.core().model().completed_steps.len());
        }
    }
    ...
    assert!(total_time >= Duration::from_millis(25), "Sequential execution too fast: {:?}", total_time);
}
...
async fn test_mixed_coordination_patterns() {
    ...
    let mixed_command = Command::batch([
        ...
        Command::sequence([
            Command::effect(CompositionEffect::FastLog { message: "Seq1".to_string() }),
            Command::effect(CompositionEffect::FastLog { message: "Seq2".to_string() }),
        ]),
        // Then some individual effects (default parallel execution by Shell)
        Command::parallel([
            CompositionEffect::DelayedLog { message: "Par1".to_string(), delay_ms: 10 },
            CompositionEffect::DelayedLog { message: "Par2".to_string(), delay_ms: 10 },
        ]),
    ]);
