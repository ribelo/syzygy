...
use syzygy::event_map::{EventMapBuilder, EventVariant};
...
fn test_simplified_api_basic_functionality() {
    ...
    let result = unsafe { event_map.dispatch(create_event, &mut model) };
...
fn test_complex_dispatch_scenario() {
    ...
    // Create user
    unsafe { event_map.dispatch(AppEvent::UserCreated(UserCreated {
        id: 1,
        name: "Alice".to_string()
    }), &mut model) };

    // Create post
    let post_result = unsafe { event_map.dispatch(AppEvent::PostCreated(PostCreated {
        id: 100,
        user_id: 1,
    ...
    unsafe { event_map.dispatch(AppEvent::UserUpdated(UserUpdated {
        id: 1,
    ...
    let delete_result = unsafe { event_map.dispatch(AppEvent::UserDeleted(UserDeleted { id: 1 }), &mut model) };
...
fn test_error_handling() {
    ...
    let result = unsafe { event_map.dispatch(AppEvent::UserUpdated(UserUpdated {
        id: 999,
    ...
    let result = unsafe { event_map.dispatch(AppEvent::PostCreated(PostCreated {
        id: 100,
        user_id: 999,
...
fn test_partial_handler_registration() {
    ...
    let result = unsafe { event_map.dispatch(AppEvent::UserCreated(UserCreated {
        id: 1,
        name: "Alice".to_string()
    }), &mut model) };
...
fn test_builder_pattern_fluency() {
    ...
    unsafe { event_map.dispatch(AppEvent::PostCreated(PostCreated {
        id: 100,
        user_id: 1,
...
fn test_dispatch_return_value_structure() {
    ...
    let result = unsafe { event_map.dispatch(AppEvent::UserDeleted(UserDeleted { id: 1 }), &mut model) };
...
fn test_model_state_consistency() {
    ...
    for event in events {
        ...
        _total_events += result.events.len();
    }
...
fn test_event_map_immutability() {
    ...
    let result1 = unsafe { event_map.dispatch(event.clone(), &mut model1) };
    let result2 = unsafe { event_map.dispatch(event, &mut model2) };
