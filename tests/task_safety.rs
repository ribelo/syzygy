...
async fn test_tasks_cancelled_on_context_drop() {
    ...
    // Create scope to control context lifetime
    {
        ...
        assert!(!task_completed.load(Ordering::Relaxed), "Task should not have completed yet");
    ...
    assert!(task_started.load(Ordering::Relaxed), "Task should have started");
    assert!(!task_completed.load(Ordering::Relaxed), "Task should have been cancelled, not completed");
}
...
async fn test_multiple_tasks_cancelled_on_drop() {
    ...
    // Create scope for context lifetime
    {
        ...
        // Let all tasks start
        sleep(Duration::from_millis(100)).await;
        assert_eq!(tasks_started.load(Ordering::Relaxed), TASK_COUNT, "All tasks should have started");
        assert_eq!(tasks_completed.load(Ordering::Relaxed), 0, "No tasks should have completed yet");
    ...
    assert_eq!(tasks_started.load(Ordering::Relaxed), TASK_COUNT, "All tasks should have started");
    assert_eq!(tasks_completed.load(Ordering::Relaxed), 0, "All tasks should have been cancelled");
}
...
async fn test_no_use_after_free_on_context_drop() {
    ...
    assert!(task_started.load(Ordering::Relaxed), "Task should have started");
    assert!(final_access_count < 1000, "Task should have been cancelled before completing all accesses (got {final_access_count})");
    assert!(final_access_count > 0, "Task should have made some accesses before cancellation");
    
    // Verify shared data is still accessible (no use-after-free)
    assert_eq!(shared_data.load(Ordering::Relaxed), 42, "Shared data should still be accessible");
}
...
async fn test_batch_spawned_tasks_cancelled_on_drop() {
    ...
    
    {
        ...
        // Let tasks start
        sleep(Duration::from_millis(100)).await;
        assert_eq!(tasks_started.load(Ordering::Relaxed), 3, "All batch tasks should have started");
        assert_eq!(tasks_completed.load(Ordering::Relaxed), 0, "No batch tasks should have completed yet");
    ...
    sleep(Duration::from_millis(200)).await;
    assert_eq!(tasks_started.load(Ordering::Relaxed), 3, "All batch tasks should have started");
    assert_eq!(tasks_completed.load(Ordering::Relaxed), 0, "All batch tasks should have been cancelled");
}
...
async fn test_cloned_context_task_safety() {
    ...
    let cloned_ctx = {
        ...
        sleep(Duration::from_millis(50)).await;
        assert!(task1_started.load(Ordering::Relaxed), "Task 1 should have started");
        assert!(task2_started.load(Ordering::Relaxed), "Task 2 should have started");
    ...
    assert!(task1_started.load(Ordering::Relaxed), "Task 1 should have started");
    assert!(task2_started.load(Ordering::Relaxed), "Task 2 should have started"); 
    assert!(!task1_completed.load(Ordering::Relaxed), "Task 1 should have been cancelled");
    assert!(!task2_completed.load(Ordering::Relaxed), "Task 2 should have been cancelled");
}
