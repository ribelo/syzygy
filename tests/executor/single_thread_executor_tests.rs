...
use syzygy::executor::{ExecutorLifecycle, ExecutorError, SingleThreadExecutor, SyncExecutor};
use syzygy::streaming::EffectOutput;

#[derive(Debug, Clone)]
enum TestEvent { A(usize), B(&'static str) }
...
async fn single_thread_executor_executes_strict_fifo_under_deep_queue() {
    ...
    assert!(results.iter().all(|r| r.is_ok()), "all jobs should complete successfully");
    ...
    for (idx, val) in order.iter().enumerate() { assert_eq!(*val, idx); }
}
...
async fn single_thread_executor_maps_panic_to_error_and_continues() {
    ...
    let err = exec.spawn_sync(Box::new(|| -> EffectOutput<TestEvent> {
        panic!("just exploding");
    })).await.expect_err("panic should map to error");
    ...
    let ok = exec.spawn_sync(Box::new(|| {
...
async fn single_thread_executor_long_running_job_does_not_starve_followers() {
    ...
    let long_started_cl = long_started.clone();
    ...
    let short_done_cl = short_done.clone();
    ...
    tokio::time::timeout(Duration::from_secs(1), async {
        while !long_started.load(Ordering::SeqCst) { tokio::task::yield_now().await; }
    }).await.expect("long job should start");

    // Both should complete within a reasonable timeout
    tokio::time::timeout(Duration::from_secs(2), long).await.expect("long join").expect("long ok");
    tokio::time::timeout(Duration::from_secs(2), short).await.expect("short join").expect("short ok");

    assert!(short_done.load(Ordering::SeqCst), "short job should have completed (no starvation)");
}
...
async fn single_thread_executor_shutdown_and_join_semantics() {
    ...
    for h in handles { let _ = h.await.expect("pre-shutdown job should succeed"); }

    // New spawns after join should be rejected
    let res = exec.spawn_sync(Box::new(|| EffectOutput::<TestEvent>::None)).await;
    assert!(matches!(res, Err(ExecutorError::WorkerGone)), "should not accept new work after shutdown");
}
