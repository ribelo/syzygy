//! EventMap correctness tests with safety validation
//!
//! This test suite validates the EventMap implementation's correctness,
...
use syzygy::event_map::{EventVariant, EventMapBuilder};
...
#[test]
fn test_unsafe_event_map_basic_dispatch() {
    // Test basic dispatch functionality after revert
    let event_map = EventMapBuilder::<FastEvent, FastModel, FastCommand>::new()
    ...
    // Test full enum dispatch after revert
    let result1 = unsafe {
        event_map.dispatch(FastEvent::Fast1(FastEvent1 { id: 100 }), &mut model)
    };
    assert_eq!(model.processed, 1);
    assert_eq!(model.counter, 100);
    assert_eq!(model.events_by_type[0], 1);
    assert!(result1.events.is_empty());
    assert_eq!(result1.commands.len(), 1); // Should generate Log command (id % 100 == 0)

    // Test another type
    let result2 = unsafe {
        event_map.dispatch(FastEvent::Fast2(FastEvent2 { value: 2000 }), &mut model)
    };
    assert_eq!(model.processed, 2);
    assert_eq!(model.counter, 2100); // 100 + 2000
    assert_eq!(model.events_by_type[1], 1);
    assert_eq!(result2.events.len(), 1); // Should generate FastEvent3
    assert_eq!(result2.commands.len(), 1); // Should generate Trigger command

    // Verify the generated event is correct
    if let FastEvent::Fast3(fast3) = &result2.events[0] {
        assert!(fast3.flag);
    } else {
...
#[test]
fn test_unsafe_event_map_backward_compatibility() {
    // Test backward compatibility with the enum dispatch
    let event_map = EventMapBuilder::<FastEvent, FastModel, FastCommand>::new()
        .on::<FastEvent1>(handle_fast1)
    ...
    // Test unregistered handler with enum API
    let result = unsafe {
    ...
    assert!(result.events.is_empty());
    assert!(result.commands.is_empty());
    assert_eq!(model.processed, 1); // Should not have incremented
}
...
fn test_unsafe_event_map_basic_functionality() {
    ...
    // Test basic dispatch with full enum after revert
    let result = unsafe { 
        event_map.dispatch(FastEvent::Fast1(FastEvent1 { id: 42 }), &mut model) 
    };
...
fn test_unsafe_simplified_api() {
    ...
    // Test registered handlers with full enum after revert
    unsafe {
    ...
    // Test unregistered handler with full enum
    let result = unsafe {
        event_map.dispatch(FastEvent::Fast2(FastEvent2 { value: 999 }), &mut model)
    };
...
fn test_unsafe_event_map_variant_indexing() {
    ...
    let map = EventMapBuilder::<FastEvent, FastModel, FastCommand>::new()
...
fn test_unsafe_event_map_cascade_events() {
    ...
    // Test event that generates cascade events with full enum
    let result = unsafe {
        event_map.dispatch(FastEvent::Fast2(FastEvent2 { value: 2000 }), &mut model)
    };
    ...
    // Test event that generates both events and commands with full enum
    let result = unsafe {
...
fn test_unsafe_event_map_error_conditions() {
    ...
    // Test registered handler works with backward compatibility API
    let _result = unsafe {
        event_map.dispatch(FastEvent::Fast1(FastEvent1 { id: 42 }), &mut model)
    };
    assert_eq!(model.processed, 1);
...
fn test_unsafe_event_map_memory_safety() {
    ...
    let map1 = EventMapBuilder::<FastEvent, FastModel, FastCommand>::new()
        .on::<FastEvent1>(handle_fast1)
        .on::<FastEvent2>(handle_fast2)
        .build();

    let map2 = EventMapBuilder::<FastEvent, FastModel, FastCommand>::new()
        .on::<FastEvent3>(handle_fast3)
        .on::<FastEvent4>(handle_fast4)
        .build();
    ...
    // Use maps independently with full enum
    unsafe {
...
fn test_unsafe_event_map_builder_pattern() {
    ...
    let test_data = FastEvent::Fast2(FastEvent2 { value: 1500 });

    let result1 = unsafe { map1.dispatch(test_data.clone(), &mut model1) };
    let result2 = unsafe { map2.dispatch(test_data, &mut model2) };
...
fn test_unsafe_event_map_zero_copy_semantics() {
    ...
    let result = unsafe { 
        event_map.dispatch(FastEvent::Fast4(FastEvent4 { data }), &mut model) 
    };
