...
pub struct SyzygyBuilder<M, E, C, S = InitialStage>
where
    E: Clone,
{
    model: Option<M>,
    ...
    command_handler: Option<fn(CommandContext<E, Resources>, C) -> Pin<Box<dyn Future<Output = ()> + Send + 'static>>>,
    _phantom: PhantomData<S>,
}

impl<Event, Command> SyzygyBuilder<(), Event, Command, InitialStage>
where
...
impl<M, Event, Command> SyzygyBuilder<M, Event, Command, InitialStage>
where
    Event: Send + Clone + 'static,
    Command: Send + Clone + 'static,
{
...
{
    /// Set the model
    pub fn model<M>(self, model: M) -> SyzygyBuilder<M, Event, Command, InitialStage> {
        SyzygyBuilder {
            model: Some(model),
...
impl<M, E, C> SyzygyBuilder<M, E, C, InitialStage>
where
    E: Send + Clone + 'static,
    C: Send + Clone + 'static,
{
