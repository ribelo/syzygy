...
use crate::executor::{SyncExecutor, ExecutorError};
...
mod tests {
    ...
    async fn rayon_executor_executes_cpu_bound_sync_work_correctly() {
        ...
        let result = executor.spawn_sync(Box::new(|| {
        ...
        match result.unwrap() {
            EffectOutput::Future(fut) => {
                ...
                match &events[0] {
                    TestEvent::CpuWork(sum) => assert_eq!(*sum, 499500), // Sum of 0..1000
    ...
    async fn rayon_executor_runs_work_on_dedicated_rayon_threads_not_tokio_threads() {
        ...
        let result = executor.spawn_sync(Box::new(|| {
            ...
            EffectOutput::Future(async move { 
                vec![TestEvent::ThreadInfo { thread_id: thread_name, value }] 
            }.boxed())
        })).await;
            ...
            EffectOutput::Future(fut) => {
                ...
                match &events[0] {
                    TestEvent::ThreadInfo { thread_id, value } => {
                        assert!(thread_id.starts_with("syzygy-rayon-"), 
    ...
    async fn rayon_executor_maintains_performance_under_high_task_churn() {
        ...
        for iteration in 0..iterations {
            let result = executor.spawn_sync(Box::new(move || {
                // Small amount of work per task
                let value = iteration * 2;
                EffectOutput::Future(async move { vec![TestEvent::CpuWork(value as u32)] }.boxed())
            })).await;
            
            // Verify each task completes successfully
            assert!(result.is_ok(), "Task {} failed", iteration);
        }
        ...
        println!("Completed {} tasks in {:?} (avg: {:?}/task)", 
                iterations, total_time, total_time / iterations as u32);
    }
    ...
    async fn rayon_executor_handles_memory_intensive_sync_work_without_blocking_tokio() {
        ...
        let memory_task = executor.spawn_sync(Box::new(|| {
            ...
            let checksum: u32 = large_vec.iter().map(|&x| x as u32).sum();
        ...
        assert_eq!(async_result.unwrap(), 42, "Async task didn't complete properly");
    }
