    ...
    E: Send + Sync + 'static,
{
    ...
    where
        T: AsyncExecutor<E> + Send + Sync + 'static,
    {
        let key = TypeId::of::<T>();
        let erased: Arc<dyn AsyncExecutor<E> + Send + Sync> = Arc::new(exec);
        self.async_map.insert(key, erased);
    }

    pub fn insert_blocking<T>(&mut self, exec: T)
    where
        T: BlockingExecutor<E> + Send + Sync + 'static,
    {
        let key = TypeId::of::<T>();
        let erased: Arc<dyn BlockingExecutor<E> + Send + Sync> = Arc::new(exec);
        self.blocking_map.insert(key, erased);
    }

    pub fn insert_resource_blocking<T>(&mut self, exec: T)
    where
        T: ResourceBlockingExecutor<E> + Send + Sync + 'static,
    {
        let key = TypeId::of::<T>();
        let erased: Arc<dyn ResourceBlockingExecutor<E> + Send + Sync> = Arc::new(exec);
    ...
    #[must_use]
    pub fn blocking_exec<T: 'static>(&self) -> Option<Arc<dyn BlockingExecutor<E>>> {
        self.blocking_map.get(&TypeId::of::<T>()).cloned()
    }

    #[must_use]
    pub fn resource_blocking_exec<T: 'static>(
        &self,
    ) -> Option<Arc<dyn ResourceBlockingExecutor<E>>> {
    ...
    pub(crate) fn blocking_exec_by_key(&self, key: TypeId) -> Option<Arc<dyn BlockingExecutor<E>>> {
