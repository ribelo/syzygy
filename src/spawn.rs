    ...
    F: Future<Output = ()> + Send + 'static,
{
    ...
    compile_error!("syzygy requires at least one async runtime feature: enable 'tokio', 'smol', or 'async-std'");
}
...
/// ```
pub fn auto_spawn_fn() -> impl Fn(Pin<Box<dyn Future<Output = ()> + Send + 'static>>) + Clone + Send + Sync + 'static {
    #[cfg(feature = "tokio")]
...
/// # });
/// ```
pub fn auto_spawn_boxed() -> impl Fn(Pin<Box<dyn Future<Output = ()> + Send + 'static>>) + Clone + Send + Sync + 'static {
    #[cfg(feature = "tokio")]
    return spawn_tokio_boxed;
...
mod tests {
    ...
    async fn test_auto_spawn_actually_works() {
        ...
        assert!(*executed.lock().unwrap(), "Spawned async block should have executed");
    }
    ...
    async fn test_spawn_trait() {
        ...
        assert!(*executed2.lock().unwrap(), "auto_spawner should execute tasks");
    }
    ...
    async fn test_zero_cost_async_spawn() {
        ...
        assert!(*executed.lock().unwrap(), "Async block should have executed via spawn_tokio");
        ...
        assert!(*executed2.lock().unwrap(), "Async block should have executed via auto_spawn");
        ...
        assert!(*executed3.lock().unwrap(), "Async function should have executed via spawn_tokio");
    }
    ...
    async fn test_boxed_compatibility() {
        ...
        assert!(*executed.lock().unwrap(), "Boxed future should have executed");
    }
}
