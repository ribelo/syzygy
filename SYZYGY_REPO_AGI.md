...
.gitignore
agi_prompt_header.md
Cargo.toml
README.md
SYZYGY_REPO_DIGEST.md
UNLICENSE
```
...
jobs:
  test:
    ...
    - name: Run clippy
      run: cargo clippy --lib --tests --bins --all-features -- -D warnings
      if: matrix.rust == 'stable'
    ...
    - name: Build (smol feature)
      run: cargo build --no-default-features --features smol --verbose
      if: matrix.features == 'smol'
    
    - name: Build (async-std feature)
      run: cargo build --no-default-features --features async-std --verbose
      if: matrix.features == 'async-std'
    
    - name: Build (view-model feature)
      run: cargo build --features view-model --verbose
      if: matrix.features == 'view-model'
    
    - name: Build (tracing feature)
      run: cargo build --features tracing --verbose
      if: matrix.features == 'tracing'
    ...
    - name: Run tests (smol feature)
      run: cargo test --no-default-features --features smol --verbose
      if: matrix.features == 'smol'
    
    - name: Run tests (async-std feature)
      run: cargo test --no-default-features --features async-std --verbose
      if: matrix.features == 'async-std'
    
    - name: Run tests (view-model feature)
      run: cargo test --features view-model --verbose
      if: matrix.features == 'view-model'
    
    - name: Run tests (tracing feature)
      run: cargo test --features tracing --verbose
      if: matrix.features == 'tracing'
  ...
  benchmark:
    ...
    - name: Run benchmarks
      run: cargo bench --features tokio
...
where
    ...
    X: Send + 'static,
{
    drive_task_with_activity(executors, task, event_tx, effect_tx, None, None)
}
    ...
    match task {
        ...
        Task::Blocking {
            ...
            let job = Box::new(move || {
                let command =
                    panic::catch_unwind(AssertUnwindSafe(|| job())).unwrap_or_else(|_| {
                        #[cfg(feature = "tracing")]
                        tracing::error!("Blocking job panicked");
                        Command::none()
                    });
                let stats_ref = stats_cl.as_ref();
                route_command(&event_tx_cl, &effect_tx_cl, command, stats_ref);
        ...
        Task::BlockingWithResource {
            ...
            let job = Box::new(move |resource: &mut dyn Any| {
                let command = panic::catch_unwind(AssertUnwindSafe(|| job(resource)))
                    .unwrap_or_else(|_| {
                        #[cfg(feature = "tracing")]
                        tracing::error!("Blocking job with resource panicked");
                        Command::none()
                    });
                let stats_ref = stats_cl.as_ref();
                route_command(&event_tx_cl, &effect_tx_cl, command, stats_ref);
...
## Backpressure & Queues
- Core event channel: unbounded `crossbeam-channel`. `Core::pending_count()` for snapshots.
...
- Call `.with_effect_channel_capacity(..)` or `.set_config(..)` afterwards to override any defaults.
...
Counters increment whenever the shell cannot deliver events or effect steps (e.g. a consumer dropped the channel during shutdown). Use them to spot misconfigured backpressure or shutdown ordering issues.
...
use crossbeam_channel::{
    unbounded, Receiver as CoreReceiver, RecvTimeoutError as CoreRecvTimeoutError,
...
impl<E> EventSender<E> {
    ...
    /// Alias for [`send`] because the channel is unbounded.
...
fn event_channel<E>() -> (CoreSender<E>, CoreReceiver<E>) {
    unbounded()
}
    ...
    X: Send + 'static,
{
    ...
    pub fn new(event_handler: EventHandler<E, X, M>, models: M) -> (Self, EventSender<E>) {
        let (raw_tx, raw_rx) = event_channel::<E>();
...
pub mod prelude {
    ...
    #[cfg(feature = "shell")]
    pub use crate::syzygy::{Syzygy, SyzygyConfig, SyzygyProfile, SyzygyProfileSettings};
...
use crate::core::EventSender;
use crate::error::ShellError;
use crate::executor::task::drive_task_with_activity;
        ...
        #[cfg(feature = "tracing")]
        {
            ...
            if snapshot.dropped_events > 0 || snapshot.dropped_effect_steps > 0 {
                tracing::warn!(
                    ...
                    dropped_events = snapshot.dropped_events,
                    dropped_effect_steps = snapshot.dropped_effect_steps,
                    "Shell dropped work during shutdown"
                );
            } else {
